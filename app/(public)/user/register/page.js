...
export default function RegisterForm() {
  ...
  const onSubmit = (data) => {
    if (data.password !== data.confirmPassword) {
      setRegisterError("Hasła nie zgadzają się.");
      return;
    }
    ...
    createUserWithEmailAndPassword(auth, data.email, data.password)
      ...
      .catch((error) => {
        if (error.code === "auth/email-already-in-use") {
          ...
          setRegisterError("Wystąpił błąd: " + error.message);
        }
        console.error(error);
      });
      ...
      <div className="lg:grid lg:min-h-screen lg:grid-cols-12">
        ...
        <main className="flex items-center justify-center px-8 py-8 sm:px-12 lg:col-span-7 lg:px-16 lg:py-12 xl:col-span-6">
          <div className="max-w-xl lg:max-w-3xl">
            ...
            <h1 className="mt-6 text-2xl font-bold text-gray-900 sm:text-3xl md:text-4xl">Witaj</h1>

            <p className="mt-4 leading-relaxed text-gray-500">Zarejestruj się byku</p>

            {registerError && (
              <div className="mt-4 text-red-600">{registerError}</div>
            )}
